...
const TABS = [
  { label: 'Esta semana', window: 'week' },
  { label: 'Hoje',        window: 'day'  },
];
...
const Trending = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await tmdbApi.getTrending(TABS[activeTab].window);
  ...
  return (
    <Page theme={theme}>
      <Inner>
      <PageHeader>
        <PageTitle theme={theme}>Bombando</PageTitle>
        <PageSubtitle theme={theme}>O que está em alta agora no mundo todo</PageSubtitle>
      </PageHeader>
      ...
      {loading ? (
        ...
        <Grid>
          {items.map(item => (
            <MovieCard key={item.id} movie={item} type={item.media_type === 'tv' ? 'tv' : 'movie'} />
          ))}
        </Grid>
      )}
      </Inner>
